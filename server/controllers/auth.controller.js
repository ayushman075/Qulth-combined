...
const clerkWebhookListener = AsyncHandler(async (req,res)=>{
    ...
    if (eventType === "user.created" || eventType === "user.updated") {
        const user = {
          ...
          userName: userData.email_addresses?.[0]?.email_address,
...
const bloom = new BloomFilter.BloomFilter(1024,4);
...
const checkUniqueUserName = AsyncHandler(async (req,res)=>{
  ...
  if(!userName){
    return res.status(409).json(new ApiResponse(409,{},"Username is required",false));
  }
  let isExist = checkUsernameAvailability(userName);

  return res.status(200).json(new ApiResponse(200,isExist,"UserName checked successfully",true))
})
...
export {clerkWebhookListener,checkUniqueUserName,updateUserProfile,getUserProfile}